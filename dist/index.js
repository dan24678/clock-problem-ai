...
function getClockState(totalMinutes) {
    let hour = 12;
    let minute = 0;
...
async function main() {
    ...
    console.log("Sorted LED Results (by minutes ON):\n");
    ...
    console.log(`Most used LED: ${sortedResults[sortedResults.length - 1].position} - ${sortedResults[sortedResults.length - 1].segment} (${sortedResults[sortedResults.length - 1].count} minutes)`);
    console.log(`Least used LEDs: ${sortedResults.filter((r) => r.count === sortedResults[0].count).map((r) => `${r.position} - ${r.segment}`).join(", ")} (${sortedResults[0].count} minutes)`);
}
